...
//        for (i = 0; i < fftTransformedData.length; i++) {
//
//        }
...
//        Log.d(mTagName, "==============");
     ...
     * @param maxZeroValue - max zero value in fft;
     * @param maxOneValue - max one value in fft;
     * @param maxAdditionalValue - max additional symbol value in fft
     * @return - if analyzing got StartSymbol - 2, else if data symbol zero - 0,
     * else if data symbol 1 - 1, else -1.
     */
    private byte compareSymbolValue(double maxZeroValue, double maxOneValue, double maxAdditionalValue) {
//        double maxValue = Math.max(maxAdditionalValue, Math.max(maxOneValue, maxZeroValue));
//        double preMaxValue =
